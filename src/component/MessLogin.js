...
const LoginForm = () => {
   ...
   //function to call api for login request 
    ...
    //     })
    // }
    const users=useSelector(store=>store.cart.mess);
    function postData(data){
        
        users.forEach(element => {
            if(element.id===data.email){
                if(element.password===data.password){
                    localStorage.setItem("MessInfo",users.indexOf(element));
                    go_to_home();
                    return 0;
                }
                else{
    ...
    <div>
      ...
      <Formik
        ...
        //   else if (!/^[A-Za-z0-9]+\/[0-9]{2,}$/i.test(values.email)) {  
        //     errors.email = 'Invalid Mess Id';
        //   }
          if (!values.password) {
      ...
      <Link to="/messHome"><div ref={clicked}></div></Link>
    </div>
  );}
